...
! Output variables:
  ...
  REAL(kind=rx), OPTIONAL, INTENT(out) :: salprac
...
! Local variables
  ...
  REAL(kind=r8) :: aK0, aK1, aK2, aKb, aKw, aKs, aKf, aKspc
  REAL(kind=r8) :: aKspa, aK1p, aK2p, aK3p, aKsi
  REAL(kind=r8) :: aSt, aFt, aBt

  REAL(kind=rx) :: sabs1, spra1, p1, lon1, lat1
  REAL(kind=rx):: tc1, ta1, sit1, nt1
  REAL(kind=rx) :: sal1
...
!        write(*,*), 'i,depth,lat,p',i,depth,lat,p
        p = p80(depth, lat)
...
!       b) Compute "in-situ Temperature" from "Potential Temperature" (both on IPTS 68)
        tempis68 = sw_temp(sal, tempot68, p, 0.d0 )
!jh        tempis68 = sw_temp(sal, SGLE(tempot68), p, SGLE(0.d0) )
...
!       Convert given conservative temperature to in-situ temperature
        ...
        tempis90 = gsw_t_from_ct (DBLE(sabs1), DBLE(temp), DBLE(p))
        tempis68  = (tempis90 - 0.0002_r8) / 0.99975_r8
     ELSE
...
!       Convert from Absolute to Practical salinity if needed
        IF (trim(opS) == 'Sabs')  THEN
           sabs1 = ssal   ! absolute sal.
           ! If longitude is passed in
           IF (PRESENT(lon)) THEN
               p1 = p
               IF (lon .NE. 1.e20_rx) THEN
           ...
           ELSE
               ...
               nt1 = 16.0 * pt
               CALL sa2sp_chem(sabs1, ta1, tc1, nt1, sit1, spra1)
           ENDIF
           s = DBLE(spra1)
           IF (PRESENT(salprac)) salprac = SGLE(s)
        ENDIF
...
!       Apply perturbation on K
        SELECT CASE (var_index)
            ...
            CASE (5)
                aKw = aKw + abs_delta
            CASE (6)
                aKspa = aKspa + abs_delta
            CASE (7)
                aKspc = aKspc + abs_delta
            CASE (8)
                aBt = aBt + abs_delta
        END SELECT
