...
SUBROUTINE flxco2(co2flux, co2ex, dpco2,                                                    &
                  ...
                  temp, sal, alk, dic, sil, phos, kw660, xco2, Patm, dz1, lon, lat,      &
...
! Local variables
  ...
  REAL(kind=rx) :: pCO2atm, fCO2atm
  REAL(kind=rx) :: depth0, lat0
 
  ! local 1-long array version of scalar variables
  REAL(kind=r8) :: sa1, s1, p1, lon1, lat1
  REAL(kind=r8) :: tc1, ta1, sit1, nt1
  
  ! practical salinity [psu] computed when absolute saliniry is given 
  REAL(kind=rx) :: salprac
...
! Compute derived variables from input (DIC, ALK, ...)
  IF (PRESENT(lat)) THEN
    CALL vars_sprac(ph, pco2, fco2, co2, hco3, co3, OmegaA, OmegaC, BetaD, rhoSW, p, tempis,  &
              temp, sal, alk, dic, sil, phos, Patm, depth0, lat,                    &
              opCON, opT, opP, opB, opK1K2, opKf, opGAS, opS, lon, salprac              )
  ELSE
    CALL vars_sprac(ph, pco2, fco2, co2, hco3, co3, OmegaA, OmegaC, BetaD, rhoSW, p, tempis,  &
              temp, sal, alk, dic, sil, phos, Patm, depth0, lat0,                    &
              opCON, opT, opP, opB, opK1K2, opKf, opGAS, opS, lon, salprac              )
  ENDIF
...
! pCO2atm = (Patm - pH20) * xCO2,   where pH20 is the vapor pressure of seawater [atm]
  CALL x2pCO2atm(xco2, temp, salprac, Patm, pco2atm)
...
! fCO2atm = pCO2atm * fugcoeff,   where fugcoeff= exp(Patm*(B + 2.0*xc2*Del)/(R*tk) )
  CALL p2fCO2(pco2atm, temp, Patm, depth0, fco2atm)

! Compute flux, absolute rate of change of surface DIC, & Delta pCO2
     IF (co3 .eq. 1.e20_rx) THEN
!       Masked values (land)
        co2flux = 1.e20_rx
        co2ex   = 1.e20_rx
        dpco2   = 1.e20_rx
...
!       Transfer velocity for CO2 in m/s (see equation [4] in OCMIP2 design document & OCMIP2 Abiotic HOWTO)
        kwco2 = DBLE(kw660) * (660/scco2(dtemp))**0.5

!       Surface K0 [(mol/kg) / atm] at T, S of surface water
        tmp = 9345.17d0*invtk - 60.2409d0 + 23.3585d0 * LOG(tk/100.0d0)
        K0 = EXP( tmp + DBLE(salprac)*(0.023517d0 - 0.00023656d0*tk + 0.0047036e-4_r8*tk*tk) )

!       "Atmospheric" [CO2*], air-sea CO2 flux, sfc DIC rate of change, & Delta pCO2
        co2starair = K0 * DBLE(fco2atm) * 1.0e-6_r8 * DBLE(rhoSW) !Equil. [CO2*] for atm CO2 at Patm & sfc-water T,S [mol/m3]
        co2star = DBLE(co2)                                          !Oceanic [CO2*] in [mol/m3] from vars.f90
        co2flux = SGLE(kwco2 * (co2starair - co2star))               !Air-sea CO2 flux [mol/(m2 * s)]
...
SUBROUTINE pCO2atm2xCO2(pCO2atm, temp, salt, Patm, xCO2)
  !    Purpose:
  !    Compute xCO2 from arrays of pCO2atm, in situ T, S, & atm pressure
...
! INPUT variables
  ...
  REAL(kind=rx), INTENT(in) :: pCO2atm
...
! LOCAL variables:
  ...
  call vapress(temp, salt, pH20)

  dpCO2atm  = DBLE(pCO2atm)
  dPatm     = DBLE(Patm)
  dxCO2     = dpCO2atm / (dPatm - pH20)
  xCO2      = SGLE(dxCO2)
...
SUBROUTINE pistonvel(windspeed, Fice, kw660)
  !    Purpose:
...
! INPUT variables
  ...
  REAL(kind=8), INTENT(in) :: windspeed
...
! Conversion factor to convert from cm/h to m/s
  ...
  kw660 = a * windspeed**2 * (1.0d0 - Fice) * xfac
...
SUBROUTINE x2pCO2atm(xCO2, temp, salt, Patm, pCO2atm)
  !    Purpose:
  !    Compute pCO2atm from arrays of xCO2, in situ T, S, & atm pressure
...
! OUTPUT variables:
  ...
  REAL(kind=rx), INTENT(out) :: pCO2atm
...
! Compute vapor pressure of seawater [in atm]
  ...
  dxCO2     = DBLE(xCO2)
  dPatm     = DBLE(Patm)
  dpCO2atm = (dPatm - pH20) * dxCO2
  pCO2atm = SGLE(dpCO2atm)
...
SUBROUTINE phizero(gasname, temp, salt, phi0)
  !    Purpose:
  !    Compute solubilities of CFC-11, CFC-12, SF6, CO2, and N20 at 1 atm pressure, i.e., Phi0 (atm), in mol L-1 atm-1 
  !    Phi0 = K0 * Cf * (Pa0 - pH20),
  !    where
...
! Compute phi0  
  tk = 273.15d0 + DBLE(temp)     !Absolute temperature (Kelvin)
  dsalt = DBLE(salt)             
  tk100 = tk/100.d0
...
SUBROUTINE kprime(gasname, temp, salt, kp)
  !    Purpose:
  !    Compute K' of CFC-11, CFC-12, and SF6; units are in mol L-1 atm-1 
...
SUBROUTINE kzero(gasname, temp, salt, k0)
  !    Purpose:
  !    Compute K0 of CO2 and N20; units are in mol L-1 atm-1 
...
SUBROUTINE vapress(temp, salt, vpsw)
  !    Purpose:
  !    Compute vapor pressure of seawater (atm) following preocedure from Weiss & Price (1980)
...
! INPUT variables
  ...
  REAL(kind=rx), INTENT(in) :: S
!f2py optional , depend(temp) :: n=len(temp)

! OUTPUT variables:
  !> O2 saturation concentration of seawater [mol/m3] 
  REAL(kind=r8), INTENT(out) :: o2sat_molm3
