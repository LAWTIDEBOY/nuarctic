...
! REAL(kind=r8) :: dtempot, dtempot68
  ...
  TYPE(DUAL_NUM) :: K0, K1, K2, Kb, Kw, Ks, Kf, Kspc
  TYPE(DUAL_NUM):: Kspa, K1p, K2p, K3p, Ksi
  TYPE(DUAL_NUM) :: St, Ft, Bt
...
!    2) Convert potential T to in-situ T (if input is Tpot, i.e. case for models):
     IF (trim(optT) == 'Tpot' .OR. trim(optT) == 'tpot') THEN
        tempot = DUAL_NUM(DBLE(temp),(/0.0D0,0.0D0,0.0D0,0.0D0,1.0D0,0.0D0/))
...
!       When optT = 'Tinsitu', tempis is input & output (no tempot needed)
        tempis90 = DUAL_NUM(DBLE(temp),(/0.0D0,0.0D0,0.0D0,0.0D0,1.0D0,0.0D0/))
...
!       Test to indicate if any of input variables are unreasonable
        IF (       sal < 0.   &
             ...
             .OR.  sil > 1e+3 &
             .OR. phos > 1e+3) THEN
           PRINT *, 'tempot, sal,    alk,    dic,    sil,    phos =', &
...
!       Atmospheric pressure
        Patmd = DUAL_NUM(DBLE(Patm),(/0.0D0,0.0D0,0.0D0,0.0D0,0.0D0,0.0D0/))
...
!       Total pressure [atm]
        IF     (trim(opGAS) == 'Pzero'   .OR. trim(opGAS) == 'pzero') THEN
           Ptot = Patmd%x_ad_               ! total pressure (in atm) = atmospheric pressure ONLY
        ELSEIF (trim(opGAS) == 'Ppot'    .OR. trim(opGAS) == 'ppot') THEN
           Ptot = Patmd%x_ad_               ! total pressure (in atm) = atmospheric pressure ONLY
        ELSEIF (trim(opGAS) == 'Pinsitu' .OR. trim(opGAS) == 'pinsitu') THEN
           Ptot = Patmd%x_ad_ + Phydro_atm   ! total pressure (in atm) = atmospheric pressure + hydrostatic pressure
        ELSE
...
!       Salinity (in double precision and with derivative w/ respect to itelf)
        s = DUAL_NUM(DBLE(ssal),(/0.0D0,0.0D0,0.0D0,0.0D0,0.0D0,1.0D0/))
...
!       (variable 't' used here just for calculation of constants)
        t = DUAL_NUM(DBLE(temp),(/0.0D0,0.0D0,0.0D0,0.0D0,1.0D0,0.0D0/))
...
!       and derivatives with respect to temperature and salinity
        drho_sw = rho_DNAD(s, tempis68, prb)
...
!       Compute chemical variables and their derivatives
        CALL varsolver_DNAD(dph, dpco2, dfco2, dco2, dhco3, dco3, dOmegaA, dOmegaC, &
                    ...
                    Kspc, Kspa, K1p, K2p, K3p, Ksi,               & 
                    Patmd, prb, drho, opGAS                                      )

!       Save derivatives
        ph_deriv(:)     = SGLE(dph%xp_ad_)
        pCO2_deriv(:)   = SGLE(dpCO2%xp_ad_)
        fCO2_deriv(:)   = SGLE(dfCO2%xp_ad_)
        co2_deriv(:)    = SGLE(dco2%xp_ad_)
        hco3_deriv(:)   = SGLE(dhco3%xp_ad_)
        co3_deriv(:)    = SGLE(dco3%xp_ad_)
        OmegaA_deriv(:) = SGLE(dOmegaA%xp_ad_)
        OmegaC_deriv(:) = SGLE(dOmegaC%xp_ad_)
     ...
     ELSE
        ...
        hco3_deriv(:)   = 1.e20_rx
        co3_deriv(:)    = 1.e20_rx
        OmegaA_deriv(:) = 1.e20_rx
        OmegaC_deriv(:) = 1.e20_rx
