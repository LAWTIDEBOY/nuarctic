...
! -----------------
  !> (d ln CO2 / dDIC)^-1
  REAL(kind=rx), INTENT(out) :: gammaDIC
  !> (d ln H+ / dDIC)^-1
  REAL(kind=rx), INTENT(out) :: betaDIC
  !> (d ln Omega / dDIC)^-1
  REAL(kind=rx), INTENT(out) :: omegaDIC
  !> aqueous CO2* concentration, either in <b>[mol/m^3]</b> or <b>[mol/kg</b>] depending on choice for optCON
  !> (d ln CO2 / dAlk)^-1
  REAL(kind=rx), INTENT(out) :: gammaALK
  !> (d ln H+ / dAlk)^-1
  REAL(kind=rx), INTENT(out) :: betaALK
  !> (d ln Omega / dAlk)^-1
  REAL(kind=rx), INTENT(out) :: omegaALK
  !> Revelle factor, i.e., dpCO2/pCO2 / dDIC/DIC
  REAL(kind=rx), INTENT(out) :: Rf
...
!                  )
      ...
      SegleCBW = hco3 + 4*co3 + (h*Borate/(Kb + h)) + h + oh
